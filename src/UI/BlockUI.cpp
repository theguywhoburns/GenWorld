...
#include <GenWorld/Core/BlockData.h>
#include <algorithm>
#include <cctype>
#include <cstring>
#include <functional>
...
#include <GenWorld/Utils/FileDialogs.h>
#include <random>

BlockUI::BlockUI(BlockController *controller) : controller(controller) {
  parameters = BlockUtilities::BlockData(20,   // gridWidth
                                         10,   // gridHeight
                                         20,   // gridLength
                                         5.0f, // cellWidth
                                         5.0f, // cellHeight
  ...
  // Try to load default castle assets (optional - won't crash if files don't
  // exist)
  LoadDefaultCastleAssets();

  // Setup socket configurations for each block type (will work even if no
  // models are loaded)
...
void BlockUI::DisplayBasicSettings() {
  ...
  if (ImGui::CollapsingHeader("Grid Settings",
    ...
    ImGui::DragInt("Grid Width (blocks)",
                   reinterpret_cast<int *>(&parameters.gridWidth), 1, 5, 100);
    ImGui::DragInt("Grid Height (blocks)",
                   reinterpret_cast<int *>(&parameters.gridHeight), 1, 5, 50);
    ImGui::DragInt("Grid Length (blocks)",
                   reinterpret_cast<int *>(&parameters.gridLength), 1, 5, 100);
    ...
    ImGui::Text(
        "Grid size: %u x %u x %u = %u total blocks", parameters.gridWidth,
        parameters.gridHeight, parameters.gridLength,
    ...
    if (parameters.dimensionsDetected) {
      ImGui::Text("Cell size: %.2f x %.2f x %.2f units", parameters.cellWidth,
                  parameters.cellHeight, parameters.cellLength);
      ImGui::Text("World size: %.1f x %.1f x %.1f units", parameters.worldWidth,
                  parameters.worldHeight, parameters.worldLength);
    }
  }

  // Block Settings
  if (ImGui::CollapsingHeader("Block Settings",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
  ...
  // Asset Management
  if (ImGui::CollapsingHeader("Asset Management",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (!loadedAssets.empty()) {
      ...
      if (ImGui::BeginPopupModal("Clear All Assets?", NULL,
        ...
        if (ImGui::Button("Yes, Clear All", ImVec2(120, 0))) {
          ...
          parameters.socketSystem
              .Initialize(); // This now properly clears all templates and
    ...
    if (ImGui::BeginPopupModal("Model Load Error", NULL,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (loadedAssets.empty()) {
      ...
      for (const auto &asset : loadedAssets) {
        ...
        // Display asset info
        ImGui::Text("ID %d: %s", asset.id,
                    GetFileName(asset.blockPath).c_str());
        ImGui::SameLine();
  ...
  ImGui::Separator();
  ImGui::InputInt("Random Seed",
                  reinterpret_cast<int *>(&parameters.randomSeed));
}
void BlockUI::DisplaySocketEditor() {
  ...
  if (loadedAssets.empty()) {
    ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f),
                       "Load assets first to set up sockets.");
    return;
  }

  // Socket type names for UI
  const char *socketTypeNames[] = {"Empty (connects to all)",
                                   "Grass",
                                   "Stone",
                                   "Wood",
                                   "Metal",
  ...
  if (selectedBlockId >= 0) {
    ...
    auto &currentTemplate = const_cast<BlockTemplate &>(
        socketSystem.GetBlockTemplates().at(selectedBlockId));

    ImGui::Separator();
    ImGui::Text("Editing Sockets for Block %d", selectedBlockId);

    const char *faceNames[] = {"+X (Right)",  "-X (Left)",  "+Y (Top)",
    ...
    for (int face = 0; face < 6; face++) {
      ...
      if (ImGui::Combo("##sockettype", &currentType, socketTypeNames, 11)) {
        currentTemplate.sockets[face].type =
            static_cast<SocketType>(currentType);
  ...
  if (ImGui::CollapsingHeader("Socket Compatibility Rules",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::BeginTable("CompatibilityMatrix", 12,
    ...
    if (ImGui::Button("All Connect")) {
      for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 11; j++) {
          compatibility.AddRule(static_cast<SocketType>(i),
                                static_cast<SocketType>(j), true);
        }
      }
    }
    ...
    if (ImGui::Button("None Connect")) {
      for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 11; j++) {
          compatibility.AddRule(static_cast<SocketType>(i),
                                static_cast<SocketType>(j), false);
        }
      }
    }
    ...
    if (ImGui::Button("Same Only")) {
      for (int i = 0; i < 11; i++) {
        for (int j = 0; j < 11; j++) {
          bool canConnect =
              (i == j || i == 0 || j == 0); // Same type OR Empty socket
          compatibility.AddRule(static_cast<SocketType>(i),
                                static_cast<SocketType>(j), canConnect);
        }
      }
    }
  }
...
void BlockUI::OpenModelFileDialog() {
  ...
  std::string filepath = Utils::FileDialogs::OpenFile(
      "3D Model Files",
      "Model Files (*.obj;*.fbx;*.gltf;*.glb)\0*.obj;*.fbx;*.gltf;*.glb\0All "
      "Files\0*.*\0",
      windowHandle);
...
void BlockUI::OnModelLoaded(std::shared_ptr<Model> model,
                            const std::string &filepath) {
  AssetInfo newAsset = {loadedAssets.empty() ? 0 : loadedAssets.back().id + 1,
  ...
  parameters.generationSettings.blockWeights[newAsset.id] =
      parameters.generationSettings.defaultWeight;
  parameters.generationSettings.maxBlockCounts[newAsset.id] =
      -1; // Set to UNLIMITED by default
}
...
std::string BlockUI::GetFileName(const std::string &filepath) {
  ...
  return lastSlash != std::string::npos ? filepath.substr(lastSlash + 1)
                                        : filepath;
}
...
void BlockUI::DisplayBlockConstraints() {
  if (ImGui::CollapsingHeader("Block Generation Constraints",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    auto &settings = parameters.generationSettings;
    ...
    if (ImGui::BeginTable("BlockConstraints", 5,
                          ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
                              ImGuiTableFlags_Resizable)) {
      ...
      for (auto &asset : assets) {
        ...
        if (isCornerBlock) {
          ImGui::TextColored(ImVec4(0.7f, 0.5f, 1.0f, 1.0f), "%s (Corner)",
                             GetFileName(asset.blockPath).c_str());
        } else {
        ...
        if (isCornerBlock) {
          ...
          ImGui::BeginDisabled();
          ImGui::SliderFloat(("##weight" + std::to_string(asset.id)).c_str(),
                             &weight, 0.0f, 1.0f, "Corner");
          ImGui::EndDisabled();
          if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
            ImGui::SetTooltip("Corner blocks use special placement logic, "
                              "weight is not applicable");
          }
        } else {
          if (ImGui::SliderFloat(
                  ("##weight" + std::to_string(asset.id)).c_str(), &weight,
                  0.0f, 1.0f, "%.2f")) {
        ...
        if (settings.minBlockCounts.find(asset.id) ==
            settings.minBlockCounts.end()) {
        ...
        if (isCornerBlock) {
          ...
          ImGui::EndDisabled();
          if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
            ImGui::SetTooltip("Corner blocks are placed using special logic, "
                              "min count is not applicable");
          }
        } else {
          char minBuf[16];
          snprintf(minBuf, sizeof(minBuf), "%d", minCount);
          if (ImGui::InputText(("##min" + std::to_string(asset.id)).c_str(),
                               minBuf, sizeof(minBuf),
            ...
            if (settings.maxBlockCounts[asset.id] != -1 &&
              ...
              newMin = settings.maxBlockCounts[asset.id];
              snprintf(minBuf, sizeof(minBuf), "%d",
                       newMin); // update buffer if clamped
            }
        ...
        if (isCornerBlock) {
          ...
          if (maxCount == -1) {
            ...
            if (ImGui::InputText(("##max" + std::to_string(asset.id)).c_str(),
        ...
        if (isCornerBlock) {
          ...
          ImGui::Checkbox(("##unlimited" + std::to_string(asset.id)).c_str(),
                          &dummyUnlimited);
          ImGui::EndDisabled();
          if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
            ImGui::SetTooltip("Corner blocks are placed using special logic, "
                              "count limits are not applicable");
          }
        } else {
          bool unlimited = (settings.maxBlockCounts[asset.id] == -1);
          if (ImGui::Checkbox(
                  ("##unlimited" + std::to_string(asset.id)).c_str(),
                  &unlimited)) {
            settings.maxBlockCounts[asset.id] =
                unlimited ? -1
                          : std::max(settings.minBlockCounts[asset.id],
                                     totalCells / 4);
          }
        }
      }
...
void BlockUI::DisplayCastleMakerSettings() {
  ...
  if (ImGui::CollapsingHeader("Castle Maker Settings",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (castleSystemEnabled) {
      ...
      ImGui::TextColored(
          ...
          "Corner blocks must have at least 2 Wall sockets on X and Z axes");
      ...
      for (const auto &asset : loadedAssets) {
        ...
        if (ImGui::Checkbox(
                (label + "##corner" + std::to_string(asset.id)).c_str(),
                &isCorner)) {
          if (isCorner) {
            if (isValidCorner) {
              ...
              ImGui::OpenPopup(
                  ("Invalid Corner Block##" + std::to_string(asset.id))
                      .c_str());
            }
          } else {
        ...
        if (ImGui::BeginPopupModal(
                ("Invalid Corner Block##" + std::to_string(asset.id)).c_str(),
        ...
        if (!isValidCorner) {
          ...
          ImGui::TextColored(ImVec4(0.7f, 0.3f, 0.3f, 1.0f),
                             "(Invalid - needs 2+ Wall sockets on X/Z axes)");
        }
      }
...
bool BlockUI::validateCornerBlock(int blockId) const {
  ...
  // Check X axis faces
  if (blockTemplate.sockets[0].type == SocketType::WALL)
    wallCount++; // +X
  if (blockTemplate.sockets[1].type == SocketType::WALL)
    wallCount++; // -X

  // Check Z axis faces
  if (blockTemplate.sockets[4].type == SocketType::WALL)
    wallCount++; // +Z
  if (blockTemplate.sockets[5].type == SocketType::WALL)
    wallCount++; // -Z
...
void BlockUI::SetupDefaultSocketConfigurations() {
  ...
  for (const auto &asset : loadedAssets) {
    ...
    if (configIt != socketConfigurations.end()) {
      ...
      std::cout << "! No socket configuration defined for block " << asset.id
  ...
  // Basic connection rules - only specific socket types can connect to each
  // other:
  ...
  compatibility.AddRule(SocketType::WALL, SocketType::EMPTY,
                        true); // WALL can only connect to empty space
  ...
  compatibility.AddRule(SocketType::STONE, SocketType::WOOD,
                        true); // Stone can connect to wood
  compatibility.AddRule(SocketType::CUSTOM_1, SocketType::CUSTOM_3,
                        true); // Tower base to tower mid windows
  compatibility.AddRule(SocketType::CUSTOM_2, SocketType::CUSTOM_3,
                        true); // Tower mid to tower mid windows
  ...
  for (const auto &asset : loadedAssets) {
    ...
    switch (asset.id) {
    case 0:
      settings.blockWeights[asset.id] = 0.08f;
      break; // Tower base
    case 1:
      settings.blockWeights[asset.id] = 0.05f;
      break; // Tower arch
    case 2:
      settings.blockWeights[asset.id] = 0.93f;
      break; // Wall (main)
    case 3:
      settings.blockWeights[asset.id] = 0.50f;
      break; // Gate
    case 4:
      settings.blockWeights[asset.id] = 0.50f;
      break; // Wall dup
    case 5:
      settings.blockWeights[asset.id] = 0.90f;
      break; // Tower roof
    case 6:
      settings.blockWeights[asset.id] = 0.50f;
      break; // Wall dup 2
  ...
  // Only enable castle system and set corner blocks if we have corner blocks
  // loaded
  if (!cornerBlockIds.empty()) {
    ...
    std::cout << "No corner blocks loaded - castle system will remain disabled"
              << std::endl;
  }
}
...
void BlockUI::LoadDefaultCastleAssets() {
  if (!controller) {
    std::cout << "No controller available - skipping default asset loading"
              << std::endl;
    return;
  }
  ...
  for (int i = 0; i < sizeof(assetPaths) / sizeof(assetPaths[0]); ++i) {
    std::shared_ptr<Model> model = std::make_shared<Model>(
        static_cast<const char *>(assetPaths[i].c_str()));

    AssetInfo newAsset = {i, "Castle_" + std::to_string(i), assetPaths[i],
                          model};
