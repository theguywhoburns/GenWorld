...
void ShadingPanel::setOnParametersChanged(
    std::function<void(const ShadingParameters &)> callback) {
...
const ShadingParameters &ShadingPanel::getParameters() const { return params; }
...
void ShadingPanel::renderModeOverlay() {
  ...
  ImVec2 overlayPos(contentRegionMin.x + 10, contentRegionMin.y + 40);
...
void ShadingPanel::renderCollapsedPanel() {
  ...
  if (ImGui::Begin("ShadingPanelCollapsed", nullptr, flags)) {
    ...
    ImVec2 contentRegionMin = ImGui::GetWindowContentRegionMin();
    ImVec2 overlayPos(contentRegionMin.x + 10, contentRegionMin.y + 85);
    ImGui::SetCursorPos(overlayPos);
    ...
    ImGui::BeginChild("SceneViewOverlay", overlaySize, ImGuiChildFlags_Borders,
                      ImGuiWindowFlags_NoScrollbar |
                          ImGuiWindowFlags_NoScrollWithMouse |
                          ImGuiWindowFlags_NoTitleBar);
...
void ShadingPanel::renderExpandedPanel() {
  ...
  if (ImGui::Begin("ShadingPanelExpanded", nullptr, flags)) {
    ...
    if (maxHeight <= contentThreshold &&
        availableHeight - 130.0f > contentThreshold) {
...
void ShadingPanel::renderWireframeUI() {
  ...
  if (ImGui::SliderFloat("Line Width", &params.wireframeWidth, 0.1f, 5.0f,
                         "%.1f")) {
    triggerCallback();
  }
...
void ShadingPanel::renderNoLightsUI() {
  ...
  ImGui::PushStyleColor(ImGuiCol_Text, ImGui::ColorConvertU32ToFloat4(
                                           ImGui::Spectrum::GRAY600()));
  ImGui::TextWrapped(
      "This mode simulates material properties without lighting. It is useful "
      "for previewing textures and material settings without the influence of "
      "scene lighting.");
  ImGui::PopStyleColor();
...
void ShadingPanel::renderWithLightsUI() {
  ...
  // Light Direction
  if (ImGui::SliderFloat3("Light Direction", &params.lightDirection[0], -1.0f,
                          1.0f)) {
    triggerCallback();
  }
