...
void TerrainUI::DisplayUI() {
    ...
    valuesChanged |= ImGui::DragFloat("Width", &parameters.width, 0.1f, 1, 100);
    valuesChanged |= ImGui::DragFloat("Length", &parameters.length, 0.1f, 1, 100);
    valuesChanged |= ImGui::SliderInt("Division Size", &parameters.cellSize, 1, 10);
    valuesChanged |= ImGui::DragFloat("Height Multiplier", &parameters.heightMultiplier, 0.1f, 1, 100);
    valuesChanged |= ImGui::DrawCurve("easeOutSine", parameters.curvePoints);
    ...
    valuesChanged |= ImGui::SliderFloat("Scale", &parameters.scale, 0.001f, 50.0f);
    valuesChanged |= ImGui::SliderInt("Octaves", &parameters.octaves, 1, 10);
    valuesChanged |= ImGui::SliderFloat("Lacunarity", &parameters.lacunarity, 0.1f, 50.0f);
    valuesChanged |= ImGui::SliderFloat("Persistence", &parameters.persistence, 0.0f, 1.0f);
    valuesChanged |= ImGui::DragFloat2("Offset", &parameters.offset[0], 0.1f);
    valuesChanged |= ImGui::DragInt("Seed", &parameters.seed, 1, 0, 10000);
    ...
    for (size_t i = 0; i < parameters.colors.size(); i++) {
        ...
        valuesChanged |= ImGui::ColorEdit3(("Color##" + index).c_str(), &parameters.colors[i].color[0]);
        valuesChanged |= ImGui::SliderFloat(("Height##" + index).c_str(), &parameters.colors[i].height, 0.0f, 1.0f);
