#include <GenWorld/Core/TerrainData.h>

namespace TerrainUtilities {
float GenerateFalloffValue(float x, float z,
                           const FalloffParameters &falloffParams) {
  ...
  case TerrainUtilities::FalloffType::CIRCULAR:
    distanceFromCenter =
        2.0f * glm::sqrt(glm::pow(normalizedX, 2) + glm::pow(normalizedZ, 2));
    break;
  ...
  default:
    ...
    distanceFromCenter =
        2.0f * glm::max(glm::abs(normalizedX), glm::abs(normalizedZ));
    break;
  }
  ...
  float value =
      glm::pow(distanceFromCenter, falloffParams.a) /
      (glm::pow(distanceFromCenter, falloffParams.a) +
       glm::pow(falloffParams.b - falloffParams.b * distanceFromCenter,
                falloffParams.a));
