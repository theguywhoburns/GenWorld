...
void ExportMeshAsFBX(const TerrainMesh& terrain, const std::string& filename) {
    ...
    if (!ExportTerrainTextureFromGPUFBX(terrain, terrainTex)) {
        std::cerr << "Failed to export terrain texture!" << std::endl;
    }

    MeshData* terrainData = ConvertMeshToMeshDataFBX(terrain, "Terrain");
    ...
    for (const auto& pair : modelInstances) {
        ...
        for (const auto& transform : transforms) {
            ...
            for (size_t i = 0; i < meshes.size(); ++i) {
                ...
                MeshData* decoData = ConvertMeshToMeshDataWithTransformFBX(*mesh, "Deco_" + std::to_string(instanceIndex) + "_Mesh_" + std::to_string(i), transform);
