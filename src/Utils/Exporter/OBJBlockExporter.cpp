...
namespace Utils {
    ...
    void OBJBlockExporter::ExportBlockMeshAsOBJWithDialog(const BlockMesh& blockMesh, GLFWwindow* window) {
    ...
    void OBJBlockExporter::ExportBlockMeshAsOBJ(const BlockMesh& blockMesh, const std::string& filename) {
    ...
    MeshData* OBJBlockExporter::ConvertMeshToMeshData(const Mesh& mesh, const std::string& name) {
        return ExportUtils::ConvertMeshToMeshData(mesh, name);
    }

    MeshData* OBJBlockExporter::ConvertMeshToMeshDataWithTransform(const Mesh& mesh, const std::string& name, const glm::mat4& transform) {
        return ExportUtils::ConvertMeshToMeshDataWithTransform(mesh, name, transform);
    }

    aiMesh* OBJBlockExporter::ConvertMeshDataToAssimp(MeshData* data, aiScene* scene, const std::string& texturePath, const std::string& outputDir) {
        return ExportUtils::ConvertMeshDataToAssimp(data, scene, texturePath, outputDir);
    }

    aiNode* OBJBlockExporter::CreateNodeWithMesh(aiScene* scene, const std::string& nodeName, aiMesh* mesh) {
    ...
    aiScene* OBJBlockExporter::CreateSceneFromBlockMesh(const BlockMesh& blockMesh, const std::string& outputDir) {
    ...
    for (const auto& [modelPath, transforms] : instanceMap) {
        ...
        if (modelIt == modelMap.end()) continue;
        ...
        for (const auto& transform : transforms) {
            ...
            groupNode->mName = aiString("BlockGroup_" + std::to_string(instanceIndex));
            ...
            for (size_t i = 0; i < meshes.size(); ++i) {
                ...
                aiMesh* instanceMesh = ConvertMeshDataToAssimp(instanceData, scene, texture, outputDir);
                if (!instanceMesh) {
                    delete instanceData;
                    continue;
                }
