...
namespace Utils {
std::string FileDialogs::OpenFile(const char *title, const char *filter,
                                  GLFWwindow *window) {
...
std::string FileDialogs::SaveFile(const char *title, const char *filter,
                                  GLFWwindow *window) {
  if (!gtk_init_check(nullptr, nullptr)) {
    return "";
  }
  ...
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      title, nullptr, GTK_FILE_CHOOSER_ACTION_SAVE, "_Cancel",
