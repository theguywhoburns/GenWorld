...
bool UiContext::init(AppWindow *window) {
  ...
  ImGui::Spectrum::LoadFonts(
      25.0f, 30.0f); // Load regular font at 25pt, bold font at 30pt
  ...
  if (iniFile.good()) {
    ...
    std::cout << "No .ini layout file found. Using default layout."
              << std::endl;
  }
...
void UiContext::renderMenuBar() {
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Window")) {
      if (ImGui::BeginMenu("Layout")) {
        if (ImGui::MenuItem("Save Layout")) {
          std::string file = Utils::FileDialogs::SaveFile(
              "Save Layout", "Layout\0*.ini\0", window->getNativeWindow());
        ...
        if (ImGui::MenuItem("Load Layout")) {
          std::string file = Utils::FileDialogs::OpenFile(
              "Load Layout", "Layout\0*.ini\0", window->getNativeWindow());
          if (!file.empty()) {
    ...
    ImGui::Text("| Current: %s",
                mode == 0 ? "Block Generation" : "Terrain Generation");
...
void UiContext::renderSceneOverlay(float *viewMatrix, float cameraDistance) {
  ...
  if (!sceneWindow)
    return; // Scene View not found, don't render gizmo
  ...
  ImGuizmo::DrawAxisTripod(viewMatrix, nullptr, gizmoPos, gizmoSize);
...
void UiContext::defaultLayout() {
  ...
  // regions
  ImGuiID top = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Up, 0.2f,
                                            nullptr, &dockspace_id);
  ImGuiID down = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Down, 0.2f,
                                             nullptr, &dockspace_id);
  ImGuiID left = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Left, 0.35f,
                                             nullptr, &dockspace_id);
  ImGuiID right = ImGui::DockBuilderSplitNode(dockspace_id, ImGuiDir_Right,
                                              0.35f, nullptr, &dockspace_id);
  ImGuiID bottom_right =
      ImGui::DockBuilderSplitNode(right, ImGuiDir_Down, 0.4f, nullptr, &right);
