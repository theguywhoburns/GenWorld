#include <GenWorld/SocketSystem/SocketSystem.h>
#include <iostream>
...
void SocketSystem::AddBlockTemplate(const BlockTemplate &blockTemplate) {
  ...
  std::cout << "Added block template for ID " << blockTemplate.blockId
            << std::endl;
}
...
void SocketSystem::GenerateRotatedVariants() {
  ...
  for (const auto &[blockId, blockTemplate] : blockTemplates) {
    ...
    std::cout << "Generated " << variants.size()
              << " rotation variants for block " << blockId << std::endl;
  }
}

std::array<Socket, 6>
SocketSystem::RotateSockets(const std::array<Socket, 6> &original,
                            int yRotation) const {
...
std::array<Socket, 6> SocketSystem::GetRotatedSockets(int blockId,
                                                      int rotation) const {
  ...
  std::cerr << "No variant found for block " << blockId << " with rotation "
            << rotation << std::endl;
  return {};
}
